...
const RouteForm = ({ route, onClose }) => {
  ...
  const validate = () => {
    if (!routeName.trim()) return t('validation.nameRequired');
    if (!routeDescription.trim()) return t('common.description') + ' requerida.';
    if (!stationId) return 'Estación obligatoria.';
    if (!durationHours || Number(durationHours) < 1) return t('validation.durationMin');
    if (!guide.trim()) return 'Guía responsable obligatoria.';
    if (!geomWkt.trim()) return 'Geometría WKT obligatoria.';
    if (!parseLineStringWkt(geomWkt.trim())) return 'La geometría debe ser un LINESTRING WKT válido.';
    return '';
  };
