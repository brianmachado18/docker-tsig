...
const makeRouteStyles = (color) => ({
  outline: new Style({ stroke: new Stroke({ color: '#ffffff', width: 9, lineCap: 'round', lineJoin: 'round' }) }),
  line:    new Style({ stroke: new Stroke({ color, width: 5, lineCap: 'round', lineJoin: 'round' }) }),
});

const userDotStyle = new Style({
  image: new CircleStyle({ radius: 8, fill: new Fill({ color: '#1a73e8' }), stroke: new Stroke({ color: '#ffffff', width: 2.5 }) }),
});

const makeDotStyle = (color) => new Style({
  image: new CircleStyle({ radius: 9, fill: new Fill({ color }), stroke: new Stroke({ color: '#ffffff', width: 2.5 }) }),
});

// ── Helpers ───────────────────────────────────────────────────────────────────

const CATEGORY_LABELS = {
  CULTURAL: 'Cultural', GASTRONOMICO: 'Gastronómico', NATURAL: 'Natural',
  HISTORICA: 'Histórica', HITORICA: 'Histórica', AVENTURA: 'Aventura', OTRO: 'Otro',
};

// Transición lineal de estados (solo los manejables manualmente)
const NEXT_STATUS = {
  pending:   'available',
  available: 'cancelled',
  cancelled: 'pending',
...
const getDestCoords = (entity, entityType) => {
  ...
  if (entityType === 'route' && entity.geomWkt) {
    try {
      ...
      if (first && Number.isFinite(first[0]) && Number.isFinite(first[1])) return [first[0], first[1]];
    } catch { /* fallback */ }
...
const getOrCreateNavLayer = (map) => {
  const existing = map.getLayers().getArray().find((l) => l.get('layerKey') === NAV_LAYER_KEY);
  if (existing) return existing;
  const layer = new VectorLayer({ source: new VectorSource(), zIndex: 60, properties: { layerKey: NAV_LAYER_KEY } });
  map.addLayer(layer);
  return layer;
};
...
const removeNavLayer = (map) => {
  if (!map) return;
  const layer = map.getLayers().getArray().find((l) => l.get('layerKey') === NAV_LAYER_KEY);
...
const renderNavRoute = (map, userCoords, destCoords, routeWkt, mode, isRoute = false) => {
  ...
  let lineGeom;
  if (routeWkt) {
    try { lineGeom = wktFormat.readGeometry(routeWkt, { dataProjection: 'EPSG:4326', featureProjection: 'EPSG:3857' }); }
    catch { lineGeom = null; }
  }
  if (!lineGeom) lineGeom = new LineString([fromLonLat(userCoords), fromLonLat(destCoords)]);

  const outlineFeat = new Feature({ geometry: lineGeom });         outlineFeat.setStyle(outline);
  const lineFeat    = new Feature({ geometry: lineGeom.clone() }); lineFeat.setStyle(line);
  const userFeat    = new Feature({ geometry: new Point(fromLonLat(userCoords)) }); userFeat.setStyle(userDotStyle);
  ...
  destFeat.setStyle(new Style({
    ...
    text: isRoute ? new Text({
      text: 'Inicio del recorrido', offsetY: -18, font: 'bold 12px sans-serif',
  ...
  return (
    <button
      ...
      className={`flex flex-col items-center gap-1.5 flex-1 py-3 px-1 rounded-xl transition-all border-2 ${
        selected ? 'bg-surface-container shadow-sm scale-[1.03]' : 'bg-surface-container hover:bg-surface-container-high border-transparent'
      }`}
...
const MapFeaturePopup = ({ editable = true }) => {
  ...
  // Estado local optimista para el status (se actualiza sin esperar al store)
  const [localStatus, setLocalStatus]       = useState(null);
  const [isTransitioning, setIsTransitioning] = useState(false);

  // Sección "¿Cómo llegar?"
  const [directionOpen, setDirectionOpen]   = useState(false);
  const [directionState, setDirectionState] = useState('idle'); // idle|loading|no-location|ready
  ...
  // Reset completo al cambiar entidad o cerrar
  useEffect(() => {
    ...
    if (!isOpen || !entity) {
      removeNavLayer(useMapStore.getState().mapInstance);
    }
  ...
  const statusColor = isRoute ? (STATUS_COLORS[displayStatus] ?? '#9e9e9e') : null;
  const subtitle    = isRoute
    ? STATUS_LABELS[displayStatus] || displayStatus
    : CATEGORY_LABELS[String(entity.category || '').toUpperCase()] || entity.category;
  ...
  const handleEdit = () => {
    handleClose();
    if (isRoute) openRouteForm(entity);
    else openAttractionForm(entity);
  };
  ...
  const handleConfirmDelete = async () => {
    setIsDeleting(true);
    const ok = await deleteRoute(entity.id);
    if (ok) {
      useMapStore.getState().refreshMapLayer('routes-wfs');
      handleClose();
    } else {
      setIsDeleting(false);
      setConfirmDelete(false);
  ...
  const handleStatusTransition = async () => {
    ...
    try {
      ...
      // Agregar entrada al historial si ya está cargado
      const today = new Date().toISOString().split('T')[0];
      setHistoryEntries((prev) => prev !== null
        ? [{ id: Date.now(), status: nextStatus, date: today }, ...prev]
        : null);
      fetchRoutes();
      useMapStore.getState().refreshMapLayer('routes-wfs');
    } catch { /* muestra el estado anterior si falla */ }
    setIsTransitioning(false);
  };

  const handleToggleDirection = () => {
    if (directionOpen) { setDirectionOpen(false); return; }
    setDirectionOpen(true);
    if (directionState !== 'idle') return; // ya en curso o terminado

    const dest = getDestCoords(entity, entityType);
    if (!dest) { setDirectionState('no-location'); return; }

    const mySession = sessionIdRef.current;
    setDirectionState('loading');

    const timer = window.setTimeout(() => {
      if (sessionIdRef.current === mySession) setDirectionState('no-location');
    }, 20000);
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        } catch { /* OSRM falló — igual mostramos cards */ }
        if (sessionIdRef.current === mySession) setDirectionState('ready');
      },
      () => {
        clearTimeout(timer);
        if (sessionIdRef.current === mySession) setDirectionState('no-location');
  ...
  const handleSelectMode = (mode) => {
    ...
    if (map) renderNavRoute(map, userCoords, dest, navRoutes?.[mode]?.wkt, mode, isRoute);
  };
  ...
  return (
    <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-50 w-[340px] bg-surface-container-lowest rounded-2xl shadow-2xl border border-outline-variant overflow-hidden">
      ...
      <div className="flex items-start justify-between px-4 pt-4 pb-3 gap-2">
        <div className="min-w-0">
          ...
          <p className="text-xs text-on-surface-variant mt-0.5 flex items-center gap-1.5">
            {statusColor && <span className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: statusColor }} />}
            <span className="capitalize">{subtitle}</span>
          </p>
        </div>
        <button onClick={handleClose} className="flex-shrink-0 p-1 rounded-full hover:bg-surface-container text-on-surface-variant" type="button">
          <span className="material-symbols-outlined text-[18px]">close</span>
        </button>
      </div>

      {/* ── Sección admin (solo rutas editables) ── */}
      {editable && isRoute && (
        <div className="border-t border-outline-variant">
          ...
          {transitionCfg && (
            <button
              ...
              style={{ color: STATUS_COLORS[NEXT_STATUS[displayStatus]] }}
            >
              {isTransitioning
          ...
          {/* Fila de acciones: editar, eliminar, historial */}
          <div className="flex items-center border-t border-outline-variant/60 divide-x divide-outline-variant/60">
            ...
            >
              ...
              Historial
              <span className="material-symbols-outlined text-[14px] transition-transform" style={{ transform: historyOpen ? 'rotate(180deg)' : 'none' }}>expand_more</span>
            </button>
          </div>
                ...
                <ul className="space-y-2">
                  {historyEntries.map((entry) => (
                    <li key={entry.id} className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] text-outline">{formatDate(entry.date)}</span>
                    </li>
                  ))}
                </ul>
              )}
          ...
          <div className="px-4 pb-4">
            {directionState === 'no-location' ? (
              <p className="text-xs text-outline text-center py-2">
                Activá la ubicación para ver la ruta y tiempos estimados
              </p>
            ) : (
              <>
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <p className="text-[11px] text-outline">
                    {directionState === 'loading'
                      ? 'Calculando rutas...'
                      : selectedMode ? 'Camino en el mapa' : 'Tocá un modo para ver el camino'}
                  </p>
                </div>
                <div className="flex gap-2">
                  <TimeCard icon="directions_car"  label="Auto"      mode="driving" minutes={navRoutes?.driving?.minutes}  loading={directionState === 'loading'} selected={selectedMode === 'driving'}  onClick={() => handleSelectMode('driving')}  />
                  <TimeCard icon="directions_bike" label="Bicicleta" mode="cycling" minutes={navRoutes?.cycling?.minutes}  loading={directionState === 'loading'} selected={selectedMode === 'cycling'}  onClick={() => handleSelectMode('cycling')}  />
                  <TimeCard icon="directions_walk" label="Caminando" mode="walking" minutes={navRoutes?.walking?.minutes}  loading={directionState === 'loading'} selected={selectedMode === 'walking'}  onClick={() => handleSelectMode('walking')}  />
                </div>
              </>
            )}
          </div>
